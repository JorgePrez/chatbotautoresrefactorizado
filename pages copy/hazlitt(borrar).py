        ...
        if datos:
            for item in datos:
                ...
                with st.container():
                    ...
                    def on_delete_chat(chat_id):
                        DynamoDatabase.delete(chat_id, session, author)
                        if st.session_state.get(f"chat_id_hayek") == chat_id:
                            st.session_state[f"messages_hayek"] = []
                            st.session_state[f"chat_id_hayek"] = ""
                            st.session_state[f"new_chat_hayek"] = False    
                        st.rerun()
